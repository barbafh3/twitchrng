// Stream related types
export const FETCH_STREAMS = 'FETCH_STREAM';
export const FETCH_STREAMS_BY_TAG = 'FETCH_STREAM_BY_TAG';
export const FETCH_USERS = 'FETCH_USERS'
export const FETCH_TAGS = 'FETCH_TAGS';
export const SAVE_TAGS_TO_JSON = 'SAVE_TAGS_TO_JSON';
export const LOAD_TAGS = 'LOAD_TAGS';
export const LOAD_TAG = 'LOAD_TAG';
export const SET_PLAYER_STATE = 'SET_PLAYER_STATE';
export const GET_TAG_BY_NAME = 'GET_TAG_BY_NAME';
export const FETCH_GAMES = 'FETCH_GAMES';
export const SAVE_GAMES_TO_JSON = 'SAVE_GAMES_TO_JSON';
export const LOAD_GAMES = 'LOAD_GAMES';
export const LOAD_GAME = 'LOAD_GAME';

// Session related types
export const TWITCH_AUTH = 'TWITCH_AUTH';
